...
class RigControl(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.cbxConAS.clicked.connect(self.srtStation)
        self.ui.cbxConNA.clicked.connect(self.srtStation)
        self.ui.cbxConSA.clicked.connect(self.srtStation)
        self.ui.cbxConAF.clicked.connect(self.srtStation)
        self.ui.cbxConOZ.clicked.connect(self.srtStation)
        self.ui.cbxConAU.clicked.connect(self.srtStation)
    ...
    def onFinished(self, vfo, mode, width, pwr, gain):
        ...
        # check if value is convertible to float, in this case it is conv. to int too.
        try:
            float(pwr)
            if pwr != self.pwr1:
                self.pwr1 = pwr
    ...
    def setBand(self, strBandLo, strBandHi):
        ...
        # SET WHERE for Frequency
        strWhereFreq = "freq >= " + strBandLo + " AND freq <= " + strBandHi

        # Set WHERE for Continent        
        if self.ui.cbxConEU.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"EU"'
            else:
                strWhereCont += " OR continent == " + '"EU"'
        if self.ui.cbxConAS.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"AS"'
            else:
                strWhereCont += " OR continent == " + '"AS"'
        if self.ui.cbxConNA.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"NA"'
            else:
                strWhereCont += " OR continent == " + '"NA"'
        if self.ui.cbxConSA.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"SA"'
            else:
                strWhereCont += " OR continent == " + '"SA"'
        if self.ui.cbxConAF.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"AF"'
            else:
                strWhereCont += " OR continent == " + '"AF"'
        if self.ui.cbxConOZ.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"OZ"'
            else:
                strWhereCont += " OR continent == " + '"OZ"'
        if self.ui.cbxConAU.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"AU"'
            else:
                strWhereCont += " OR continent == " + '"AU"'
        if self.ui.cbxConAN.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"AN"'
            else:
                strWhereCont += " OR continent == " + '"AN"'
        if self.ui.cbxConUD.isChecked():
            if strWhereCont == "":
                strWhereCont = " continent == " + '"undef"'
            else:
                strWhereCont += " OR continent == " + '"undef"'

        # Prepare complete SQL String
        if strWhereFreq == "" and strWhereCont == "":
            strCursor = "SELECT * FROM stations " + " " + strOrder
            print(strCursor)
            #self.curs.execute("SELECT * FROM stations " + " " + strOrder)
        elif strWhereFreq != "" and strWhereCont == "":
            strCursor = "SELECT * FROM stations WHERE " + strWhereFreq + " " + strOrder
            print(strCursor)
            #self.curs.execute("SELECT * FROM stations WHERE " + strWhereFreq + " " + strOrder)
        elif strWhereFreq == "" and strWhereCont != "":
            strCursor = "SELECT * FROM stations WHERE " + strWhereCont + " " + strOrder
            print(strCursor)
            #self.curs.execute("SELECT * FROM stations WHERE " + strWhereCont + " " + strOrder)
        elif strWhereFreq != "" and strWhereCont != "":
            strCursor = "SELECT * FROM stations WHERE " + strWhereFreq + " AND ( " + strWhereCont + " ) " + strOrder
            print(strCursor)
            #self.curs.execute("SELECT * FROM stations WHERE " + strWhereCont + " AND ( " + strWhereCont + " ) " + strOrder)
        
        # Set Cursor in Database
        try:
            self.curs.execute(strCursor)
